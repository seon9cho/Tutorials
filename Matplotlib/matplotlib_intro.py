...
def prob1():
    ...
    for i in range(100, 1001, 100):
    	ar = np.append(ar, var_of_means(i))
    plt.plot(ar)
    plt.show()
...
def prob4():
    ...
    plt.suptitle("Different Sine Functions")
...
def prob5():
    ...
    ax2 = plt.subplot(122)
    x2 = fars[:,0]
    ax2.hist(x2, bins=np.arange(0,24))
...
def prob6():
    ...
    lim = np.array([-2*np.pi, 2*np.pi, -2*np.pi, 2*np.pi])

    plt.subplot(121)
    plt.pcolormesh(X, Y, g, cmap="coolwarm")
    plt.colorbar()
    plt.axis(lim)

    plt.subplot(122)
    plt.contour(X, Y, g, 15, cmap="magma")
    plt.colorbar()
    plt.axis(lim)
